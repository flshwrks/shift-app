...
export default function StaffRequestsPage() {
  ...
  useEffect(() => {
    fetchRequests();
    const channel = supabase.channel(`staff-requests-${storeId}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'shift_requests', filter: `store_id=eq.${storeId}` }, fetchRequests)
      // shift_request_targets には store_id 列が無いため filter は付けない
