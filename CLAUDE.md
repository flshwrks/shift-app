...
`flshwrks/shift-app` は **Public**。会社に中身を見せられることが説明材料になるため公開のままにしているが、
**ここに入れたものは取り消せない**（gitの履歴・fork・キャッシュに残る）。

**入れてはいけないもの:**

- **本番URL**（`https://....vercel.app`）。ドメインが分かれば、あとは店舗IDだけで公開シフト表に届く
- **具体的な店舗ID**を含むパス。文章では必ず `/s/[storeSlug]/...` とプレースホルダで書く
- **実名**（スタッフ・管理者・取引先）
- **スタッフに配る文面**そのもの（LINEノート・掲示物）

**置き場所:** 配布用の文面や本番URLは `~/.claude/シフト管理アプリ/資料/` 配下（**git管理外**）に置く。

`npm run check:leak` がCIで検査する。**ルールを文章で書くだけでは再発する**ので、
新しい種類の「出してはいけないもの」に気づいたら `scripts/check-public-leak.ts` に足すこと。

経緯: 2026-08-17〜09-11、`docs/staff-line-note.txt` に本番URL・店舗ID・実名が入ったまま
約1か月公開されていた。そのURLはログイン不要でスタッフの氏名と勤務予定が見える画面に直結していた
（`docs/SECURITY.md`）。
